...
var JSLike = (() => {
  ...
  var ThrowSignal = class {
    constructor(value) {
      this.value = value;
    }
  };

  // src/errors/enhanced-error.js
  ...
  function findSimilar(target, candidates, maxDistance = 3) {
    ...
    return suggestions.sort((a, b) => a.distance - b.distance).slice(0, 3).map((s) => s.name);
  }
  ...
  var EnhancedTypeError = class extends TypeError {
    ...
    getFormattedMessage() {
      ...
      if (this.availableMethods.length > 0) {
        ...
        const more = this.availableMethods.length > 10 ? ` (and ${this.availableMethods.length - 10} more)` : "";
        formatted += `
...
Did you mean: ${this.suggestions.join(", ")}?`;
  ...
  function createMethodNotFoundError(objectName, methodName, objectValue) {
    ...
    const message = `TypeError when calling method '${methodName}' on object '${objectName}': Expected: function, Received: undefined`;
  ...
  var Interpreter = class _Interpreter {
    ...
    async evaluateAsync(node, env) {
      ...
      if (node.type === "BlockStatement") {
        ...
        for (const statement of node.body) {
          ...
          if (result instanceof ReturnValue || result instanceof ThrowSignal || result instanceof BreakSignal || result instanceof ContinueSignal) {
            return result;
          }
        }
        return result;
      ...
      if (node.type === "ReturnStatement") {
        const value = node.argument ? await this.evaluateAsync(node.argument, env) : void 0;
      ...
      if (node.type === "LogicalExpression") {
        ...
        if (node.operator === "&&") {
          ...
          return left !== null && left !== void 0 ? left : await this.evaluateAsync(node.right, env);
        }
      ...
      if (node.type === "UnaryExpression") {
        if (node.operator === "delete" && node.argument.type === "MemberExpression") {
          ...
          const prop = node.argument.computed ? await this.evaluateAsync(node.argument.property, env) : node.argument.property.name;
      ...
      if (node.type === "UpdateExpression") {
        if (node.argument.type === "Identifier") {
          ...
          const current2 = env.get(name);
          const numericCurrent = current2 === null || current2 === void 0 ? 0 : Number(current2);
          ...
          if (obj === null || obj === void 0) {
            throw new TypeError(
              `Cannot read properties of ${obj} (reading '${node.argument.computed ? await this.evaluateAsync(node.argument.property, env) : node.argument.property.name}')`
            );
          }
          const prop = node.argument.computed ? await this.evaluateAsync(node.argument.property, env) : node.argument.property.name;
          let current2 = obj[prop];
          const numericCurrent = current2 === null || current2 === void 0 ? 0 : Number(current2);
          const newValue = node.operator === "++" ? numericCurrent + 1 : numericCurrent - 1;
          obj[prop] = newValue;
          return node.prefix ? newValue : numericCurrent;
    ...
    evaluate(node, env) {
      ...
      switch (node.type) {
        ...
        case "ReturnStatement":
          return new ReturnValue(node.argument ? this.evaluate(node.argument, env) : void 0);
        case "IfStatement":
    ...
    tryConvertBlockToObjectLiteral(block, env) {
      ...
      if (block.body.length === 1 && block.body[0].type === "ExpressionStatement") {
        ...
        if (expr.type === "SequenceExpression" && expr.expressions.every((e) => e.type === "Identifier")) {
          const obj2 = {};
          for (const identifier of expr.expressions) {
            obj2[identifier.name] = env.get(identifier.name);
          }
          return obj2;
        }
        if (expr.type === "Identifier") {
          const obj2 = {};
          obj2[expr.name] = env.get(expr.name);
          return obj2;
        }
      }
      ...
      switch (node.operator) {
        ...
        case "delete":
          if (node.argument.type === "MemberExpression") {
            ...
            const prop = node.argument.computed ? this.evaluate(node.argument.property, env) : node.argument.property.name;
            return delete obj[prop];
          }
          return true;
        default:
    ...
    evaluateUpdateExpression(node, env) {
      if (node.argument.type === "Identifier") {
        ...
        if (obj === null || obj === void 0) {
          throw new TypeError(
            `Cannot read properties of ${obj} (reading '${node.argument.computed ? this.evaluate(node.argument.property, env) : node.argument.property.name}')`
          );
        }
        const prop = node.argument.computed ? this.evaluate(node.argument.property, env) : node.argument.property.name;
        let current2 = obj[prop];
    ...
    applyCompoundAssignment(operator, left, right) {
      ...
      const leftVal = isNumericOp && left === void 0 ? 0 : left;
      switch (operator) {
        case "+=":
          if (left === void 0 && typeof right === "number") {
            return 0 + right;
          }
          return left + right;
        case "-=":
    ...
    evaluateLogicalExpression(node, env) {
      ...
      if (node.operator === "&&") {
        ...
        return left !== null && left !== void 0 ? left : this.evaluate(node.right, env);
      }
      throw new Error(`Unknown logical operator: ${node.operator}`);
    }
    evaluateConditionalExpression(node, env) {
      const test = this.evaluate(node.test, env);
      return test ? this.evaluate(node.consequent, env) : this.evaluate(node.alternate, env);
    }
    ...
    evaluateObjectExpression(node, env) {
      ...
      for (const prop of node.properties) {
        if (prop.type === "SpreadElement") {
          ...
          const key = prop.key.type === "Identifier" && !prop.computed ? prop.key.name : this.evaluate(prop.key, env);
    ...
    evaluateVariableDeclaration(node, env) {
      const isConst = node.kind === "const";
      for (const declarator of node.declarations) {
        const value = declarator.init ? this.evaluate(declarator.init, env) : void 0;
    ...
    bindObjectPattern(pattern, value, env, isConst = false) {
      ...
      for (const prop of pattern.properties) {
        if (prop.type === "RestElement") {
          const assignedKeys = pattern.properties.filter((p) => p.type !== "RestElement").map((p) => p.key.name || p.key.value);
          const restObj = {};
    ...
    evaluateBlockStatement(node, env) {
      ...
      for (const statement of node.body) {
        ...
        if (result instanceof ReturnValue || result instanceof BreakSignal || result instanceof ContinueSignal || result instanceof ThrowSignal) {
          return result;
        }
      }
      return result;
  ...
  function createGlobalEnvironment(env) {
    ...
    env.define("last", (array, n) => {
      if (n === void 0) return array[array.length - 1];
    ...
    env.define("get", (obj, path, defaultValue) => {
      ...
      return current2 !== void 0 ? current2 : defaultValue;
    });
    env.define("set", (obj, path, value) => {
      ...
      for (let i = 0; i < keys.length - 1; i++) {
        const key = keys[i];
        if (!(key in current2)) current2[key] = {};
        current2 = current2[key];
      }
  ...
  var WangInterpreter = class {
    ...
    get globalContext() {
      return {
        variables: this.persistentEnv?.vars || /* @__PURE__ */ new Map()
      };
    }
    ...
    async execute(code, initialEnv = void 0, userOptions = {}) {
  ...
  var InMemoryModuleResolver = class {
    constructor() {
  ...
  var isTopLevelAwait = containsModuleSyntax;
...
//# sourceMappingURL=jslike.browser.js.map
